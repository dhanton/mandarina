...
public:
	...
	struct TreePosition {
        TreePosition(const BoundingBody<Number>& _bbox, TreeNode<Object>* _node) :
			bounding_box(_bbox), node(_node) {
			current_child = ChildPosition::kNone;
		}

        BoundingBody<Number> bounding_box;
		TreeNode<Object>* node;
		ChildPosition current_child;
	};

	ForwardTreeTraversal();
    void StartAt(TreeNode<Object>* root, const BoundingBody<Number>& root_bounds);
	int GetDepth() const; ///< starting from 0
	TreeNode<Object>* GetNode() const;
    const BoundingBody<Number>& GetNodeBoundingBox() const;
	void GoTopLeft();
	void GoTopRight();
	void GoBottomRight();
	void GoBottomLeft();
...
class
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::
Impl {
public:
	enum class QueryType {kIntersects, kInside, kContains, kEndOfQuery};

	Impl();
    void Acquire(typename Quadtree<Number, Object, BoundingBoxExtractor>::Impl* quadtree,
        const BoundingBody<Number>* query_region, QueryType query_type);
	void Release();
...
private:
    ...
    typename Quadtree<Number, Object, BoundingBoxExtractor>::Impl* quadtree_;
    ...
    BoundingBody<Number> query_region_;
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
class
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::
Impl {
public:
    ...
    const BoundingBody<Number>& GetBoundingBox() const; ///< loose sense bounds
	int GetSize() const;
	void Clear();
	void ForceCleanup();
...
private:
	...
	using QueryPoolContainer =
        std::deque<typename Quadtree<Number, Object, BoundingBoxExtractor>::Query::Impl>;
    ...
    BoundingBody<Number> bounding_box_;
...
ForwardTreeTraversal() :
    position_(BoundingBody<Number>(), nullptr), depth_(0) {
}

template <typename NumberT, typename ObjectT>
void
	detail::ForwardTreeTraversal<NumberT, ObjectT>::
StartAt(TreeNode<Object>* root, const BoundingBody<Number>& root_bounds) {
...
void
	detail::ForwardTreeTraversal<NumberT, ObjectT>::
GoTopLeft() {
    RotatingRect<Number>& bbox = position_.bounding_box.rect;
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::Impl::
Impl() : quadtree_(nullptr), query_region_(),
	query_type_(QueryType::kEndOfQuery),
    free_ride_from_level_(Quadtree<Number, Object, BoundingBoxExtractor>::Impl::kInternalMaxDepth) {
}

template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
void
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::Impl::
Acquire(typename Quadtree<Number, Object, BoundingBoxExtractor>::Impl* quadtree,
        const BoundingBody<Number>* query_region, QueryType query_type) {
	assert(IsAvailable());
	...
	free_ride_from_level_ =
        Quadtree<Number, Object, BoundingBoxExtractor>::Impl::kInternalMaxDepth;
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
void Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::Impl::Next() {
	assert(!IsAvailable());
	assert(!EndOfQuery());
		...
		if (object_iterator_ == traversal_.GetNode()->objects.end()) {
			do {
				...
				case detail::ChildPosition::kBottomLeft:
                    ...
                    if (traversal_.GetDepth() > quadtree_->maximal_depth_ && quadtree_->running_queries_ == 1) {
                        typename detail::TreeNode<Object>::ObjectContainer& objects = traversal_.GetNode()->objects;
					...
					if (traversal_.GetDepth() > 0) {
						...
						if (traversal_.GetNode()->objects.empty() &&
                            ...
                            quadtree_->bounding_box_ = BoundingBody<Number>();
						}
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
bool Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::Impl::CurrentObjectFits() const {
    BoundingBody<Number> object_bounds;
    BoundingBoxExtractor::ExtractBoundingBody(GetCurrent(), &object_bounds);
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
auto Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Query::Impl::CurrentNodeFits() const -> FitType {
    const BoundingBody<Number>& node_bounds = traversal_.GetNodeBoundingBox();
    BoundingBody<Number> extended_bounds = node_bounds;

    Number half_width = (Number)((typename detail::MakeDistance<Number>::Type)node_bounds.rect.width / 2);
    Number half_height = (Number)((typename detail::MakeDistance<Number>::Type)node_bounds.rect.height / 2);

    extended_bounds.rect.width = (Number)(extended_bounds.rect.width * 2);
    extended_bounds.rect.height = (Number)(extended_bounds.rect.height * 2);
    extended_bounds.rect.left = (Number)(extended_bounds.rect.left - half_width);
    extended_bounds.rect.top = (Number)(extended_bounds.rect.top - half_height);
...
template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Impl::
Impl() :
    root_(nullptr), bounding_box_(),
...
auto
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Impl::
QueryIntersectsRegion(const BoundingBody<Number>& region) -> Query {
...
auto
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Impl::
QueryInsideRegion(const BoundingBody<Number>& region) -> Query {
	typename Query::Impl* query_impl = GetAvailableQueryFromPool();
...
auto
    Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Impl::
QueryContainsRegion(const BoundingBody<Number>& region) -> Query {
	typename Query::Impl* query_impl = GetAvailableQueryFromPool();
...
DeleteTree() {
    ...
    bounding_box_ = BoundingBody<Number>();
	number_of_objects_ = 0;
	maximal_depth_ = kInternalMinDepth;
}

template <typename NumberT, typename ObjectT, typename BoundingBoxExtractorT>
ObjectT** Quadtree<NumberT, ObjectT, BoundingBoxExtractorT>::Impl::InsertIntoTree(Object* object) {
    BoundingBody<Number> object_bounds;
    BoundingBoxExtractor::ExtractBoundingBody(object, &object_bounds);

    bool is_circle = object_bounds.isCircle;

    if (is_circle) {
        assert(object_bounds.circle.radius >= 0);
    } else {
        assert(object_bounds.rect.width >= 0);
        assert(object_bounds.rect.height >= 0);
        assert(object_bounds.rect.left <= object_bounds.rect.left + object_bounds.rect.width);
        assert(object_bounds.rect.top <= object_bounds.rect.top + object_bounds.rect.height);
    }

    Number maximal_object_extent;
    sf::Rect<Number> globalBounds = object_bounds.rect.getGlobalBounds();

    if (is_circle) {
        maximal_object_extent = object_bounds.circle.radius * (Number) 2;
    } else {
        maximal_object_extent = globalBounds.width >= globalBounds.height ? globalBounds.width : globalBounds.height;
    }
    ...
    Number object_center_x;
    Number object_center_y;

    if (is_circle) {
        object_center_x = object_bounds.circle.center.x;
        object_center_y = object_bounds.circle.center.y;
    } else {
        object_center_x = (Number)(object_bounds.rect.left + (Number)((typename detail::MakeDistance<Number>::Type)object_bounds.rect.width / 2));
        object_center_y = (Number)(object_bounds.rect.top + (Number)((typename detail::MakeDistance<Number>::Type)object_bounds.rect.height / 2));
    }


	if (root_ != nullptr) {
        assert(number_of_objects_ >= 0);
        assert(bounding_box_.rect.width > 0);
        assert(bounding_box_.rect.width == bounding_box_.rect.height);

		int depth_increase = 0;
        while (!bounding_box_.Contains(object_center_x, object_center_y) || maximal_object_extent > bounding_box_.rect.width) {
            Number previous_size = bounding_box_.rect.width;
            bounding_box_.rect.width = (Number)(bounding_box_.rect.width * 2);
            bounding_box_.rect.height = bounding_box_.rect.width;

            Number previous_half = (Number)((typename detail::MakeDistance<Number>::Type)previous_size / 2);
            Number bb_center_x = (Number)(bounding_box_.rect.left + previous_half);
            Number bb_center_y = (Number)(bounding_box_.rect.top + previous_half);
			...
			if (object_center_x <= bb_center_x) {
                bounding_box_.rect.left = (Number)(bounding_box_.rect.left - previous_size);

				if (object_center_y <= bb_center_y) {
                    bounding_box_.rect.top = (Number)(bounding_box_.rect.top - previous_size);
            ...
            assert(bounding_box_.rect.left < bounding_box_.rect.left + bounding_box_.rect.width);
            assert(bounding_box_.rect.top < bounding_box_.rect.top + bounding_box_.rect.height);
            ...
            assert(!std::is_integral<Number>::value || globalBounds.width < std::numeric_limits<Number>::max() / 8 * 7);
            assert(!std::is_integral<Number>::value || globalBounds.height < std::numeric_limits<Number>::max() / 8 * 7);
		}

		detail::ForwardTreeTraversal<Number, Object> trav;
		trav.StartAt(root_, bounding_box_);
		do {
            const BoundingBody<Number>& node_bounds = trav.GetNodeBoundingBox();
            assert(node_bounds.Contains(object_center_x, object_center_y));

            Number maximal_bb_extent = node_bounds.rect.width >= node_bounds.rect.height ? node_bounds.rect.width : node_bounds.rect.height;
            Number half_bb_extent = (Number)((typename detail::MakeDistance<Number>::Type)maximal_bb_extent / 2);
			assert(maximal_object_extent <= maximal_bb_extent);

            if (maximal_object_extent > half_bb_extent || trav.GetDepth() >= maximal_depth_) {
				break;
			}

            Number node_center_x = (Number)(node_bounds.rect.left + (Number)((typename detail::MakeDistance<Number>::Type)node_bounds.rect.width / 2));
            Number node_center_y = (Number)(node_bounds.rect.top + (Number)((typename detail::MakeDistance<Number>::Type)node_bounds.rect.height / 2));
...
#ifdef MANDARINA_DEBUG
        BoundingBody<Number> effective_bounds = trav.GetNodeBoundingBox();
        Number half_width = (Number)((typename detail::MakeDistance<Number>::Type)effective_bounds.rect.width / 2);
        Number half_height = (Number)((typename detail::MakeDistance<Number>::Type)effective_bounds.rect.height / 2);

        effective_bounds.rect.width = (Number)(effective_bounds.rect.width * 2);
        effective_bounds.rect.height = (Number)(effective_bounds.rect.height * 2);
        effective_bounds.rect.left = (Number)(effective_bounds.rect.left - half_width);
        effective_bounds.rect.top = (Number)(effective_bounds.rect.top - half_height);

        assert(effective_bounds.Contains(object_bounds));
#endif

        typename detail::TreeNode<Object>::ObjectContainer& objects = trav.GetNode()->objects;
		...
		{
            bounding_box_.rect.width = (Number)((typename detail::MakeDistance<Number>::Type)maximal_object_extent * 2 * 7 / 8);
            bounding_box_.rect.height = bounding_box_.rect.width;

            Number extent_half = (Number)((typename detail::MakeDistance<Number>::Type)bounding_box_.rect.width / 2);
            bounding_box_.rect.left = (Number)(object_center_x - extent_half);
            bounding_box_.rect.top = (Number)(object_center_y - extent_half);
